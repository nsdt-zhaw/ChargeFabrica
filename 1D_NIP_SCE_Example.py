...
def solve_for_excitation_site(excitation_index, n_values, p_values, a_values, c_values, phi_values):
    ...
    dt, MaxTimeStep, desired_residual, DampingFactor, NumberofSweeps, max_timesteps = 1e-7, 1e-6, 1e-12, 0.05, 1, 2000
...
def simulate_device(output_dir):
    ...
    for start in range(0, len(excitation_indices), chunk_size):
        ...
        chunk_results = Parallel(n_jobs=chunk_size, backend="multiprocessing")(delayed(solve_for_excitation_site)(excitation_index, n_values, p_values, a_values, c_values, phi_values) for excitation_index in chunk_excitation_indices)
