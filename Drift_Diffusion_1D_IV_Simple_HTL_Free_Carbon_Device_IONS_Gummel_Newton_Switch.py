...
#Here we use the Gummel method for the first 50 timesteps and then switch to the Newton method to accelerate the final convergence.
import os
...
def solve_for_voltage(voltage, n_values, p_values, a_values, c_values, phi_values):
    ...
    dt, MaxTimeStep, desired_residual, DampingFactor, NumberofSweeps, max_iterations = 1e-7, 1e-6, 1e-10, 0.05, 1, 2000
    ...
    while SweepCounter < max_iterations and residual > desired_residual:
        ...
        for i in range(NumberofSweeps):
            ...
            if SweepCounter<50:
                #Start using the Gummel method, which doesn't need a good initial guess.
                ...
                #Accelerate final convergence using Newton's method
