...
def solve_for_voltage(voltage, n_values, p_values, a_values, c_values, phi_values):
    ...
    dt, MaxTimeStep, desired_residual, DampingFactor, NumberofSweeps, max_iterations = 1e-9, 1e-6, 1e-15, 0.1, 1, 2000
    ...
    while SweepCounter < max_iterations and residual > desired_residual or SweepCounter < 500:
        ...
        for i in range(NumberofSweeps):
            ...
            residual = eqn.sweep(dt=dt, solver=solver) + eqp.sweep(dt=dt*10, solver=solver)
