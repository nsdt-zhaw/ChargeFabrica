# -*- coding: utf-8 -*-
#This code computes the EQE of a NIP perovskite solar cell with mobile ions
#Use PlottingResults1D_EQE.py to plot the results
import os
...
(Recombination_Langevin_values, Recombination_Bimolecular_values, anion_mob_values, cation_mob_values, Nd_values, Na_values) = flatten_and_smooth_all([Recombination_Langevin_values, Recombination_Bimolecular_values, anion_mob_values, cation_mob_values, Nd_values, Na_values],0.00)
...
tau_p_bulk = 100 * 1.00e-9
tau_n_bulk = 100 * 1.00e-9
...
def solve_for_wavelength(voltage, n_values, p_values, a_values, c_values, phi_values, GenRate_values_default):
    ...
    gen_rate = cell_variable(mesh, "generation minus recombination rate")
...
def simulate_device(output_dir):
    ...
    Tested_Wavelengths = np.arange(StartingWavelength, FinalWavelength, 10.00)

    print("Tested Wavelengths: ", Tested_Wavelengths)

    chunk_size = len(Tested_Wavelengths)
    ...
    for start in range(0, len(Tested_Wavelengths), chunk_size):
        ...
        PhotonFluxArrayFinal = []
        PhotonFluxArrayOriginal = []
        GenerationArray = []
        GenerationArrayOriginal = []
        for i in range(len(chunk_wavelengths)):
            # Find the argument of the closest SolarSpectrumWavelength to start
            closest_start_index = np.argmin(np.abs(SolarSpectrumWavelength - chunk_wavelengths[i]))

            GenRate_values_default_location = map_semiconductor_property(DeviceArchitechture, 'GenRate')

            SolarSpectrumWavelengthEQE = SolarSpectrumWavelength[closest_start_index - 5:closest_start_index + 5]
            ...
            GenerationArrayOriginal.append(GenRate_values_defaultSolarSpec)
            PhotonFluxArrayFinal.append(PhotonFluxArray)
            PhotonFluxArrayOriginal.append(PhotonFluxArraySolarSpec)

        PhotonFluxArrayFinal = np.array(PhotonFluxArrayFinal)
        PhotonFluxArrayOriginal = np.array(PhotonFluxArrayOriginal)
        GenerationArray = np.array(GenerationArray)
        GenerationArrayOriginal = np.array(GenerationArrayOriginal)

        PhotonFluxArrayOriginalSplit = PhotonFluxArrayOriginal
        GenerationArrayOriginal = np.sum(GenerationArrayOriginal, axis=0)
        PhotonFluxArrayOriginal = np.sum(PhotonFluxArrayOriginal, axis=0)

        GenerationArray = GenerationArray + GenerationArrayOriginal  # Add the perturbation on top of solar spectrum

        PhotonFluxArrayOriginal = np.expand_dims(PhotonFluxArrayOriginal, axis=0)
        GenerationArrayOriginal = np.expand_dims(GenerationArrayOriginal, axis=0)
        GenerationArray = np.concatenate((GenerationArrayOriginal, GenerationArray), axis=0)

        PhotonFluxArrayFinal = PhotonFluxArrayFinal + PhotonFluxArrayOriginal  # Add the perturbation on top of solar spectrum
